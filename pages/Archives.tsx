...
import { MOCK_SEASONS, MOCK_STANDINGS, MOCK_DRIVERS, MOCK_SERIES, MOCK_RACES, GT3_R1_RESULT } from '../constants';
...
const SeasonCard: React.FC<{ season: Season, onClick: () => void, language: string }> = ({ season, onClick, language }) => {
  const { t } = useLanguage();
  ...
  const getDriver = (id: string): Driver | undefined => MOCK_DRIVERS.find(d => d.id === id);
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl font-racing font-bold text-white mb-2 leading-tight italic tracking-tighter">{season.name}</h3>
          ...
          return (
            <div key={series.id} className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-white truncate max-w-[100px]">{driver?.name || 'N/A'}</span>
              </div>
            </div>
          );
        })}
                          ...
                          {topThree.map((standing, index) => {
                            ...
                            return (
                              <div key={standing.driverId} className="flex items-center gap-4 p-4 bg-slate-950/40 rounded-2xl border border-slate-800/50">
                                ...
                                <img src={driver?.avatar} className="w-12 h-12 rounded-xl border border-slate-800" alt={driver?.name} />
                                <div className="flex-1">
                                  <p className="font-bold text-white">{driver?.name}</p>
                                  <p className="text-[10px] text-slate-500 uppercase tracking-widest">{driver?.country}</p>
                                </div>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            {raceSession.length > 0 ? raceSession.slice(0, 3).map((row, idx) => (
                              <div key={idx} className="bg-slate-950/60 p-3 rounded-xl border border-slate-800/50 flex flex-col items-center">
                                ...
                                <span className="text-[9px] font-bold text-white truncate w-full text-center">{row.display_name.split(' ')[0]}</span>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {activeSeasons.map(season => (
                  <SeasonCard key={season.id} season={season} language={language} onClick={() => setSelectedSeason(season)} />
                ))}
              </div>
            </section>
          )}

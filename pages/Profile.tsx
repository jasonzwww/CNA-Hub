        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 text-slate-400 mb-8 font-medium">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-2.5 h-2.5 rounded-full ${user.status === 'Online' ? 'bg-green-500' : 'bg-red-500 animate-pulse'}`} />
                <span className="uppercase text-xs font-bold">{user.status}</span>
              </div>
              <div className="px-3 py-1 bg-slate-950 rounded-lg border border-slate-800 text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <Hash size={12} /> {t.iracingId}: {user.id}
              </div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">{t.rank}</p>
                <p className="text-2xl font-racing font-bold text-white italic">#34</p>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">iRating</p>
                <p className="text-2xl font-racing font-bold text-red-500 italic">{user.iRating}</p>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">Safety</p>
                <p className="text-2xl font-racing font-bold text-blue-400 italic">{user.safetyRating}</p>
              </div>
              <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800">
                <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">CNA Points</p>
                <p className="text-2xl font-racing font-bold text-amber-500 italic">{totalPoints}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          <section className="bg-slate-900/50 rounded-3xl border border-slate-800 p-6">
            <h3 className="text-lg font-racing font-bold text-white uppercase italic mb-6 flex items-center gap-3">
              <Activity className="text-red-500" /> {t.careerStats}
            </h3>
          ...
          <section className="bg-slate-900/50 rounded-3xl border border-slate-800 p-6">
            <h3 className="text-lg font-racing font-bold text-white uppercase italic mb-6 flex items-center gap-3">
              <Users className="text-red-500" /> {t.myTeams}
            </h3>
            <div className="space-y-4">
                ...
                <Link to="/matchmaking" key={team.id} className="block group">
                  <div className="p-4 bg-slate-950/40 rounded-xl border border-slate-800/50 group-hover:border-red-600/50 transition-all">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-[10px] font-black text-red-500 uppercase">{team.targetGoal}</span>
                    </div>
                    <div className="flex items-center gap-3 text-[10px] text-slate-500 font-bold uppercase">
                      <Car size={12} /> {team.car}
                    </div>
                  </div>
                </Link>
              )) : (
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-slate-900/50 rounded-[32px] border border-slate-800 p-8 h-full flex flex-col">
            <h3 className="text-xl font-racing font-bold text-white uppercase italic mb-8 flex items-center gap-3">
              <History className="text-red-500" /> {t.myCareer}
            </h3>
            ...
            <div className="flex-1 space-y-4">
              {myRecentRaces.length > 0 ? myRecentRaces.map(race => (
                <div key={race.id} className="bg-slate-950/60 p-6 rounded-2xl border border-slate-800/50 flex flex-col md:flex-row items-center gap-6 group">
                  <div className="w-full md:w-32 h-20 rounded-xl overflow-hidden bg-slate-900 border border-slate-800 shrink-0">
                    <img src={race.image} className="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-opacity" alt={race.track} />
                  </div>
                  <div className="flex-1 text-center md:text-left">
                    <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                      <span className="text-[10px] font-black text-red-500 uppercase tracking-widest">{race.seriesName}</span>
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">• {formatLocalDateOnly(race.dateTime)}</span>
                    </div>
                    <h4 className="text-lg font-racing font-bold text-white italic truncate">{race.track}</h4>
                  </div>
                  <div className="flex items-center gap-8 pr-4">
                    <div className="text-center">
                      <p className="text-[9px] font-bold text-slate-500 uppercase mb-1">Finish</p>
                      <p className="text-xl font-racing font-bold text-white">#4</p>
                    </div>
                    <div className="text-center">
                      <p className="text-[9px] font-bold text-slate-500 uppercase mb-1">Inc</p>
                      <p className="text-xl font-racing font-bold text-slate-400">2x</p>
                    </div>
                    <ArrowRight className="text-slate-700 group-hover:text-red-500 transition-all" />
                  </div>
                </div>
              )) : (
                <div className="flex-1 flex flex-col items-center justify-center py-20 text-center opacity-40">
                  ...
                  <p className="font-bold uppercase tracking-widest text-xs text-slate-500">{t.noParticipation}</p>
                </div>
              )}
            </div>

            <button className="mt-8 w-full py-4 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-2xl text-xs uppercase tracking-widest transition-all">
              Load Full Career History
            </button>
          </section>
        </div>
      </div>

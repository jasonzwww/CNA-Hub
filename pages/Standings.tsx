...
import { MOCK_STANDINGS, MOCK_DRIVERS, MOCK_SERIES } from '../constants';
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filteredStandings.map((standing, index) => {
                const driver = MOCK_DRIVERS.find(d => d.id === standing.driverId);
                const rank = index + 1;
                  ...
                  <tr key={standing.driverId} className="hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <img src={driver?.avatar} className="w-10 h-10 rounded-lg border border-slate-800" alt={driver?.name} />
                        <div>
                          <div className="font-bold text-white group-hover:text-red-500 transition-colors">{driver?.name}</div>
                          <div className="text-[10px] text-slate-500 uppercase tracking-widest">{driver?.country}</div>
                        </div>
                      </div>
                    </td>

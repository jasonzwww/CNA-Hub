...
import { MOCK_ENDURANCE_TEAMS, MOCK_DRIVERS } from '../constants';
                      ...
                      {team.members.map(memberId => {
                        ...
                        return (
                          <div key={memberId} className="flex items-center gap-4 p-4 bg-slate-950/40 rounded-2xl border border-slate-800/50 hover:bg-slate-950 transition-colors shadow-sm">
                            <img src={driver.avatar} className="w-12 h-12 rounded-xl object-cover border border-slate-800" alt={driver.name} />
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-bold text-white truncate">{driver.name}</p>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] text-slate-500 uppercase font-bold">{driver.country}</span>
                                <div className="w-1 h-1 rounded-full bg-slate-700" />
                                <span className="text-[10px] text-red-500 font-black">{driver.iRating} iR</span>
                              </div>
                            </div>
                          </div>
                        );

...
// Import specific D3 modules to resolve type errors associated with the umbrella namespace import
import { 
  select, 
  geoOrthographic, 
  geoPath, 
...
const Globe: React.FC = () => {
  ...
  const rotationRef = useRef<[number, number]>([0, -10]); // [longitude, latitude]
  ...
  useEffect(() => {
    ...
    // Use named geoOrthographic function
    ...
    // Use named geoPath function
    const path = geoPath().projection(projection as any);

    // Use named geoGraticule function
    ...
    function render() {
      ...
      driversGroup.selectAll('circle')
        ...
        .style('display', (d: any) => {
          // Use named geoDistance function
    ...
    // Use named json function for fetching data
    json('https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson').then((data: any) => {
      ...
      driversGroup.selectAll('circle')
        ...
        .attr('stroke', '#fff')
        .attr('stroke-width', 2)
        .attr('class', 'cursor-pointer hover:fill-white transition-colors')
        .style('filter', 'drop-shadow(0 0 5px rgba(239, 68, 68, 0.8))')
        .on('click', (event, d: Driver) => {
    ...
    // Use named d3Drag (aliased drag) to create behavior and rename variable to avoid collision
    ...
    // Use named timer function and rename local variable to avoid shadowing the import
        ...
        {selectedDriver && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 bg-slate-900/90 backdrop-blur-md border border-red-500/50 rounded-2xl p-5 shadow-2xl animate-in zoom-in-95 fade-in duration-200 pointer-events-auto">
                ...
                <div className="flex flex-col items-center text-center">
                    <img src={selectedDriver.avatar} className="w-16 h-16 rounded-xl border-2 border-red-500 mb-3" />
                    <h3 className="font-racing font-bold text-white text-lg leading-tight mb-1">{selectedDriver.name}</h3>
                    <p className="flex items-center gap-1 text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4">
                        <MapPin size={10} className="text-red-500" /> {selectedDriver.country}
                    </p>
                    
                    <div className="grid grid-cols-2 gap-2 w-full">
                        <div className="bg-slate-950/50 p-2 rounded-lg border border-slate-800">
                            <div className="flex items-center justify-center gap-1 text-[8px] font-bold text-slate-500 uppercase mb-1">
                                <Trophy size={8} className="text-amber-500" /> iRating
                            </div>
                            <div className="text-sm font-racing font-bold text-white">{selectedDriver.iRating}</div>
                        </div>
                        <div className="bg-slate-950/50 p-2 rounded-lg border border-slate-800">
                            <div className="flex items-center justify-center gap-1 text-[8px] font-bold text-slate-500 uppercase mb-1">
                                <Shield size={8} className="text-blue-500" /> SR
                            </div>
        ...
        {!isAutoRotating && !selectedDriver && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 pointer-events-none">
                <div className="bg-slate-950/60 backdrop-blur-sm px-4 py-1.5 rounded-full border border-slate-800 flex items-center gap-2">
                    ...
                    <button 
                        onClick={(e) => { e.stopPropagation(); setIsAutoRotating(true); }}
                        className="pointer-events-auto ml-2 text-red-500 hover:text-red-400 text-[10px] font-black underline underline-offset-2 uppercase"
                    >
                        {t.resume}
                    </button>
                </div>
      ...
      <div className="mt-12 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
        {MOCK_DRIVERS.map(driver => (
            <button 
                ...
                className={`flex items-center gap-3 bg-slate-900/40 p-3 rounded-xl border transition-all hover:bg-slate-800/60 ${selectedDriver?.id === driver.id ? 'border-red-500/50 bg-red-500/5' : 'border-slate-800'}`}
            >
                <img src={driver.avatar} className="w-8 h-8 rounded-lg" />

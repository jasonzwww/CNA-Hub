...
const Globe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    json('https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson').then((data: any) => {
      ...
      driversGroup.selectAll('circle').data(MOCK_DRIVERS).enter().append('circle').attr('r', 8).attr('fill', '#ef4444').attr('stroke', '#fff').attr('stroke-width', 2).attr('class', 'cursor-pointer hover:fill-white transition-colors').style('filter', 'drop-shadow(0 0 5px rgba(239, 68, 68, 0.8))').on('click', (event, d: Driver) => {
